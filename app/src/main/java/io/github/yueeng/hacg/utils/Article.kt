...
@Parcelize
data class Comment(
    val id: Int, val parent: Int, val content: String, val user: String, val face: String,
    var moderation: Int, val time: String, val children: MutableList<Comment>, val depth: Int = 1
) : Parcelable {
    companion object {
    ...
    constructor(e: Element, depth: Int = 1) :
            this(
                ID.find(e.attr("id"))?.let { it.groups[1]?.value }?.toInt() ?: 0,
